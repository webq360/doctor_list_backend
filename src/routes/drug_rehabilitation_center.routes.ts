...
import DrugRehabilitationCenter from '../models/drug_rehabilitation_center.model';
import CenterService from '../models/center_service.model';
import Gallery from '../models/gallery.model';
import { protect, authorize } from '../middleware/auth.middleware';

const router = Router();
const CENTER_TYPE = 'drug_rehab';
...
router.get('/', async (req: Request, res: Response) => {
  ...
  const centers = await DrugRehabilitationCenter.find(filter).sort({ createdAt: -1 });
  res.json(centers);
});

router.post('/', protect, authorize('admin'), async (req: Request, res: Response) => {
  try {
    const center = await DrugRehabilitationCenter.create(req.body);
    res.status(201).json(center);
  } catch (err: any) { res.status(500).json({ message: err.message }); }
});

router.put('/:id', protect, authorize('admin'), async (req: Request, res: Response) => {
  const center = await DrugRehabilitationCenter.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
router.delete('/:id', protect, authorize('admin'), async (req: Request, res: Response) => {
  await DrugRehabilitationCenter.findByIdAndDelete(req.params.id);
